...
def on_getting_recommendations(style, ratings, ids):
    rating_input = {
        'm'+str(ids[i]["MovieID"]): int(rating) for i, rating in enumerate(ratings) if rating is not None
    }

    rating_series= pd.Series(rating_input)

    recommended_movies = get_recommended_movies(rating_series)
